...
public class BioVisualizerConvEdgeRenderer extends AbstractShapeRenderer{

	public static final String EDGE_DEFAULT_DRAW_TYPE = "EDGE_DEFAULT_DRAW_TYPE";
	public static final String EDGE_EXTENDED_BEZIER_DRAW_TYPE = "EDGE_EXTENDED_BEZIER_DRAW_TYPE";
	 ...
	 */
	public BioVisualizerConvEdgeRenderer() {
	 ...
	 * @param vis 
	 */
	public BioVisualizerConvEdgeRenderer(int edgeType, Visualization vis) {
	 ...
	 */
	public BioVisualizerConvEdgeRenderer(int edgeType, int arrowType) {
		this();
	...
	protected Shape getRawShape(VisualItem item) {
		...
		synchronized (visualization) {
			...
			int m_edgeArrow = item.getInt(EDGE_TYPE);
			...
			double x1, double y1, double x2, double y2) 
	{
		...
		case EDGE_EXTENDED_BEZIER_DRAW_TYPE:
			...
			break;
		case EDGE_DEFAULT_DRAW_TYPE:
			defaultType(eitem, cp, x1, y1, x2, y2);
			break;
		case EDGE_LINEAR_DRAW_TYPE:
			linearType(eitem, cp, x1, y1, x2, y2);
			break;
		default:
			extendedType(eitem, cp, x1, y1, x2, y2);
			break;
	...
	private void linearType(EdgeItem eitem, Point2D[] cp, 
			double x1, double y1, double x2, double y2){
		cp[0].setLocation(x1,y1);
		cp[1].setLocation(x2,y2);
	}
	
	private void defaultType(EdgeItem eitem, Point2D[] cp, 
			double x1, double y1, double x2, double y2){
	...
	private void extendedType(EdgeItem eitem, Point2D[] cp, 
			double x1, double y1, double x2, double y2){

//		System.out.println("P1\t" + x1 + "\t" + y1);
//		System.out.println("P2\t" + x2 + "\t" + y2);
//		System.out.println(eitem.getSourceItem().get(LayoutUtils.TYPE) + "\t" + eitem.getSourceItem().getX() + "\t"+ eitem.getSourceItem().getY());
//		System.out.println(eitem.getTargetItem().get(LayoutUtils.TYPE) + "\t" + eitem.getTargetItem().getX() + "\t"+ eitem.getTargetItem().getY());

		Point2D a = new Point2D.Double(x2, y2);
		Point2D r =  new Point2D.Double(x1, y1);
		Point2D b = getMoreDistante(eitem.getSourceItem(),a);
		...
		cp[0].setLocation(x1, y1);
		cp[1].setLocation(newPoint.getX(), newPoint.getY()) ;


		//		eitem.getSourceItem().get(LayoutUtils.TYPE)
		//		System.out.println("##############");
		//
		//		System.out.println("P1: "+  x1 + "\t" + y1);
		//		System.out.println("control 1: " + cp[0]);
		//		System.out.println("control 2: " + cp[1]);
		//		
		//		System.out.println("P2: "+ x2 + "\t" + y2);
		//		System.out.println("##############");
	}
	...
	private Point2D getMoreDistante(NodeItem r, Point2D point){
		double distance = 0.0;
		Point2D moreDistant = point;
		
			Iterator it = r.outNeighbors();

			while(it.hasNext()){
				TableNodeItem node = (TableNodeItem)it.next();
				
					Double x = node.getDouble(LayoutUtils.MY_X);
					Double y = node.getDouble(LayoutUtils.MY_Y);
					double distanceC = point.distance(x, y);
					if(distanceC> distance){
						distance = distanceC;
						moreDistant = new Point2D.Double(x, y);
					}
