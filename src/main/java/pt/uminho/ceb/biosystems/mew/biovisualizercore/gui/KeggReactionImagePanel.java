...
public class KeggReactionImagePanel extends JPanel implements ComponentListener, ReactionListener {
	...
	public KeggReactionImagePanel(Container container) {
		super();
	...
	public KeggReactionImagePanel getThis() {
		return this;
	}
	...
	private void rescaleImage(final BufferedImage image) {
		if (image != null) {
			...
			if ((imageWidth > parentWidth) && (parentWidth > 0)) {
				...
				System.err.println("[IF] - I=["+imageWidth +" x "+imageHeight+"] / P=["+"PARENT = "+parentWidth +" x "+parentHeight+"]");
				...
				System.err.println("[ELSE IF] - I=["+imageWidth +" x "+imageHeight+"] / P=["+"PARENT = "+parentWidth +" x "+parentHeight+"]");
				...
				System.err.println("[ELSE] - I=["+imageWidth +" x "+imageHeight+"] / P=["+"PARENT = "+parentWidth +" x "+parentHeight+"]");
				rescaledImage = GraphicsUtils.getScaledInstance(image, parentWidth, parentHeight, RenderingHints.VALUE_INTERPOLATION_BILINEAR, true);
			}
			if (rescaledImage != null) {
				ImageIcon icon = new ImageIcon(rescaledImage);
				_currentImage = rescaledImage;
